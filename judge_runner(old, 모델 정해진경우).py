...
def load_env() -> Dict[str, Any]:
    ...
    cfg = {
        ...
        # B
        "B_VENDOR": os.getenv("MODEL_B_VENDOR", "openai").strip().lower(),
...
def to_gemini_request(messages, temperature=0.7, top_p=0.95, max_tokens=1024):
    # OpenAI messages[{role, content}] -> Gemini payload
    ...
    for m in messages:
        ...
        if role == "system":
            continue
        gem_role = "user" if role == "user" else "model"  # assistant -> model
        contents.append({
            "role": gem_role,
...
async def call_gemini_chat(
    client: httpx.AsyncClient,
    base_url: str,          # e.g., https://generativelanguage.googleapis.com
    model: str,             # e.g., gemini-2.5-flash
    api_key: str,           # 안전상 헤더로도 같이 보냄
    system_prompt: str,
    user_prompt: str,
    force_json: bool = False,
) -> Tuple[bool, Dict[str, Any] or str, Dict[str, Any]]:
    ...
    started = time.perf_counter()
    try:
        # OpenAI messages 형태로 만들어서 to_gemini_request에 전달
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user",   "content": user_prompt},
        ]
        ...
        # client 기본 헤더에 x-goog-api-key가 이미 있어도 무방하지만, 안전하게 한 번 더 지정
...
def call_gemini(messages, model, base_url, api_key,
                temperature=0.7, top_p=0.95, max_tokens=1024, stream=False):
    url = f"{base_url}/v1beta/models/{model}:generateContent"
    headers = {
        "Content-Type": "application/json",
        "x-goog-api-key": api_key,   # ← 쿼리스트링 ?key= 대신 헤더 권장
    }
    payload = to_gemini_request(messages, temperature, top_p, max_tokens)
...
async def process_one(
    ...
    if cfg.get("B_VENDOR") == "gemini":
        b_coro = call_gemini_chat(
            clients["B"], cfg["B_BASE"], cfg["B_MODEL"], cfg["B_KEY"],
            prompts["B_SYS"], query, force_json=False
        )
    else:
        b_coro = call_chat(
        ...
        with jsonl_path.open("w", encoding="utf-8") as fw:
            for i, row in enumerate(tqdm(rows, desc="Evaluating", unit="q")):
                try:
                    ...
                    # 코드/로직 에러는 바로 중단 (HTTP 오류는 process_one 내부에서 ok=False로 이미 흡수됨)
